...
namespace FreeContentCatalogApplication
{
    public class Program
    {
        ...
        {
            StringBuilder output = new StringBuilder();Catalog cat = new Catalog();
    ...
    public class Content : IComparable, IContent
    {
        ...
        {
            this.Type = type;
            this.Title = commandParams[(int)CatalogProductItems.Title];
            this.Author = commandParams[(int)CatalogProductItems.Author];
            this.Size = Int64.Parse(commandParams[(int)CatalogProductItems.Size]);
            this.URL = commandParams[(int)CatalogProductItems.Url];
        }
    ...
    public class Command : ICommand
    {
        ...
        public CommandType ParseCommandType(string commandName)
        {
            ...
            {
                throw new FormatException("Command cannot contain \";\" and \"\n\", as well as leading or trailing whitespace.");
            }
                ...
                default:
                    {
                        ...
                        {
                            throw new ArgumentException("Not complete command, inccorect items!"); //todo: Review exception
                        }

                        if (commandName.ToLower().Contains("find") || commandName.ToLower().Contains("update"))
                        {
                            throw new ArgumentException("Not complete command, please provide correct arguments"); //todo: Review exception
                        }

                        throw new IndexOutOfRangeException("Invalid command name!");
                    }
            }
